# The Luhn formula is a simple checksum formula used to validate a
...
#   modulus 10 is congruent to 0), then the number is valid according to t
# he Luhn formula; else it is not valid. So, 1111 is not valid (as shown above,
...
class Luhn
  ...
  def addends
    addend_array = []
    counter = 0
    number.to_s.chars.map(&:to_i).reverse.each do |num|
      if counter.odd?
        num *= 2
        num -= 9 if num >= 10
      end
      addend_array.unshift(num)
      counter += 1
    end
    addend_array
  end
  ...
  def self.create(invalid_num)
    invalid_num *= 10
    luhn_two = Luhn.new(invalid_num)
    final_digit = (10 - luhn_two.checksum % 10) % 10
    invalid_num + final_digit
  end
end
