...
def light(switches)
  light_switches = create(switches)
  return_arr = []

  light_switches.each do |key, value|
    placeholder = 0
    while placeholder <= light_switches.length
        placeholder += key
      if light_switches[placeholder]== 'on'
        light_switches[placeholder] = 'off'
      elsif light_switches[placeholder] == 'off'
        light_switches[placeholder] = 'on'
      end
